...
def resource_path(relative_path):
    """
    Obtém o caminho absoluto para o recurso, funciona tanto em ambiente de desenvolvimento quanto após o empacotamento com PyIntaller.
    """
    try:
...
def calcular():
    """
    Realiza o cálculo dos valores trogonométricos (seno, cosseno e tangente) do ângulo fornecido e atualiza as labels com os resultados.
    """
    try:
...
frame_entrada = tk.Frame(janela, text="Ângulo (0 à`90):", font=('Arial', 14), bg="#f0f0f0")
...
entrada_angulo = tk.Entry(frame_entrada, width=3, justify='center', font=('Arial', 16), bd=0, highlightthickness=0, relief='flat', bg="#f0f0f0", fg='red', validate="key", validatecommand=(validacao, '%P'))
entrada_angulo.pack()

linha = tk.Frame(janela, bg="#f0f0f0")
linha.pack(pady=(0, 5))

botao_calcular = tk.Button(frame_botoes, text="Calcular", command=calcular, font=('Arial', 12), bg="#d9d9d9", relief='flat', bd=0, highlightthickness=0)
botao_calcular.pack(side=tk.LEFT, padx=10)

botao_limpar = tk.Button(frame_botoes, text="Limpar", command=limpar, font=('Arial', 12), bg="#d9d9d9", relief='flat', bd=0, highlightthickness=0)
